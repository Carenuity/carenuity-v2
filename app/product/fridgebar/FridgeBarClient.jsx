...
export default function FridgeBarClient() {
  const { t } = useLanguage();
  const c = t.fridgeBar;
      ...
      >
        ...
        <div className="container relative">
          <nav className="flex items-center gap-2 pt-6 pb-4 text-sm">
            ...
            <span className="text-green-300/40">/</span>
            <span className="font-medium text-white">FridgeBar</span>
          </nav>
        ...
        <div className="container">
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Image
                src="/images/products/fridgebar/fridge.png"
                alt="FridgeBar fridge monitor"
                width={0}
                height={0}
                sizes="100vw"
                className="w-full h-auto rounded-2xl mb-6"
              />
              <h3 className="text-lg font-bold text-[#0d2137] dark:text-white mb-2">
                {c.problemTitle}
              </h3>
              ...
              {/* Solution */}
              <h3 className="text-xl font-bold text-[#0d2137] dark:text-white mb-3">
                {c.solutionTitle}
              </h3>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-10">
                {c.signals.map((sig) => (
                  <div
                    key={sig.color}
                    className="flex items-center gap-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 px-4 py-3 flex-1"
                  >
                    <span
                      className={`w-4 h-4 rounded-full shrink-0 ${signalColors[sig.color] || "bg-gray-400"}`}
                    />
                    <div>
                      <span className="text-sm font-semibold text-[#0d2137] dark:text-white">
                        {sig.color}:{" "}
                      </span>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-10">
                <ul className="space-y-3">
                  {c.alerts.map((item) => (
                    ...
                    >
                      <span className="mt-0.5 w-5 h-5 rounded-full bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center shrink-0">
                        <svg
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
                {c.benefits.map((b) => (
                  ...
                  >
                    <h3 className="text-sm font-bold text-[#0d2137] dark:text-white mb-1">
                      {b.title}
                    </h3>
              ...
              <div className="space-y-3 mb-10">
                {c.howSteps.map((step, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4"
                  >
